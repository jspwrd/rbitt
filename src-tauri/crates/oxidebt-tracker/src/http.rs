...
impl HttpTracker {
    ...
    pub(crate) fn parse_announce_response(&self, data: &[u8]) -> Result<AnnounceResponse, TrackerError> {
    ...
    pub(crate) fn announce_to_scrape_url(&self, announce_url: &str) -> Result<String, TrackerError> {
    ...
    pub(crate) fn parse_scrape_response(&self, data: &[u8]) -> Result<ScrapeResponse, TrackerError> {
        let value = decode(data)?;
